// $Header:   D:/databases/VMdb/archives/Sunrise/Weld Controller/2000XWCProj/CyGOS/MultiBoot/SerialBootTask.cpp_v   1.4   08 Jan 2015 03:03:44   AnPawar  $
//
...
void SerialBootTask::EraseCallback(int offset, int length)
{
	pprintf("Erase: %08X of %08X\n", offset, length);
	Board::TriggerWD();
}

void SerialBootTask::ProgramCallback(int offset, int length)
{
...
void SerialBootTask::BurnImageToFlash()
{
	...
	printf("Boot image: address %08X, size %08X (%8d)\n",
			(unsigned int) __USER_FLASH_ROM, bootImageLength, bootImageLength);
	flash->Unlock(__USER_FLASH_ROM, bootImageLength);
	if (!flash->Erase(__USER_FLASH_ROM, bootImageLength, EraseCallback))
...
void SerialBootTask::Tick()
{
	static int WdTimer = 0;
	WdTimer += usecPerTick;
	if(WdTimer >= 1000000)//trigger at 1 second
	{
		Board::TriggerWD();
		WdTimer -= 1000000;
	}
