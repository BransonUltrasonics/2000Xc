// $Header:   D:/databases/VMdb/archives/Sunrise/Weld Controller/2000XWCProj/CyGOS/Arch/MCF/MCF5445x/DSPI.cpp_v   1.10   11 Dec 2014 08:29:44   AnPawar  $
//
...
void DSPI::ConfigureSPIPins()
{
	...
	//PCS 3
	MCF_PAD_PAR_DMA |= MCF_PAD_PAR_DMA_PAR_DACK0_DSPI_CS3;

	// Drive strength max
	MCF_PAD_DSCR_DSPI = MCF_PAD_DSCR_DSPI_DSE_SPI(3);
	MCF_PAD_DSCR_DMA |= MCF_PAD_DSCR_DMA_DSE_DMA(3);
}
 ...
 */
void DSPI::EnableSPI(bool phase, int bits, int clock, bool polarity, bool lsb, int ctar)
{
...
//	}
	...
	ctarreg |= MCF_DSPI_DSPI_CTAR_FMSZ(bits - 1); // frame size
	ctarreg |= MCF_DSPI_DSPI_CTAR_PDT_5CLK | MCF_DSPI_DSPI_CTAR_DT(4);
	if (polarity)
		ctarreg |= MCF_DSPI_DSPI_CTAR_CPOL; // clock polarity
	if (phase)
		ctarreg |= MCF_DSPI_DSPI_CTAR_CPHA; // clock phase
	if (lsb)
		ctarreg |= MCF_DSPI_DSPI_CTAR_LSBFE; // lsb first enable
	reg->CTAR[ctar] = ctarreg | baud;
...
/*SetDelayParams: Set Delay Parameters in Control and Transfer Attribute reg DSPI_CTAR0.
 Function can be called after EnableSPI function to change the default delay parameters values
 for a slave device. The input parameters passed in this function are the settings for CTAR bits.
 Check processor manual for actual delay.
 @param pcsck : PCS to SCK delay prescaler.
 @param pasc : After SCK delay prescaler.
 @param pdt : Delay after transfer prescaler.
 @param cssck : PCS to SCK delay scaler.
 @param asc : After SCK delay scaler..
 @param dt: Delay after transfer scaler.
 */
void DSPI::SetDelayParams(int pcssck, int pasc, int pdt, int cssck, int asc,
		int dt, int ctar)
{
	uint32 ctarreg = reg->CTAR[ctar];
	ctarreg &= ~(MCF_DSPI_DSPI_CTAR_PCSSCK(0x3) | MCF_DSPI_DSPI_CTAR_PASC(0x3)
			| MCF_DSPI_DSPI_CTAR_PDT(0x3) | MCF_DSPI_DSPI_CTAR_CSSCK(0xF)
			| MCF_DSPI_DSPI_CTAR_ASC(0xF) | MCF_DSPI_DSPI_CTAR_DT(0xF));

	ctarreg |= (MCF_DSPI_DSPI_CTAR_PCSSCK(pcssck) | MCF_DSPI_DSPI_CTAR_PASC(pasc)
			| MCF_DSPI_DSPI_CTAR_PDT(pdt) | MCF_DSPI_DSPI_CTAR_CSSCK(cssck)
			| MCF_DSPI_DSPI_CTAR_ASC(asc) | MCF_DSPI_DSPI_CTAR_DT(dt));

	reg->CTAR[ctar] = ctarreg;
}
 ...
 */
void DSPI::SingleTransfer(int length, uint16 * data, int chipSelect, bool cont, int ctar)
{
	...
	for (int i = 0; i < length; i++)
	{
		...
		while (!(reg->SR & MCF_DSPI_DSPI_SR_TFFF))
			;
		int pushr = MCF_DSPI_DSPI_PUSHR_CTAS(ctar); // first CTAR register
		if (cont || (i < (length - 1)))
		...
		int chipSelect, int ctar)
{
	...
	for (int i = 0; i < length; i++)
	{
		...
		if (i == (transferLength - 1))
			cont = 0;
		int tx = 0 | MCF_DSPI_DSPI_PUSHR_CTAS(ctar) // first CTAR register
...
/**
 * 	If interrupt enabled this Interrupt Handler called after SPI frame transfer is completed.
 */
