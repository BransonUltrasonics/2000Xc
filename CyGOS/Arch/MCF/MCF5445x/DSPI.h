// $Header:   D:/databases/VMdb/archives/Sunrise/Weld Controller/2000XWCProj/CyGOS/Arch/MCF/MCF5445x/DSPI.h_v   1.5   21 May 2014 04:18:54   rjamloki  $
//
...
public:
	 ...
	 */
	void EnableSPI(bool phase, int bits, int clock, bool polarity, bool lsb, int ctar = 0);
	unsigned char ReadByte(unsigned char send, int chipSelect);
	void SingleTransfer(int length, uint16 * data, int chipSelect, bool cont, int ctar = 0);
	void LongWordTransfer(int length, uint8 * data, int maxLength,
			int chipSelect, int ctar = 0);
	void SetDelayParams(int pcssck, int pasc, int pdt, int cssck, int asc,
			int dt, int ctar = 0);
	/**
	 * 	Variables
	 */
