// $Header:   D:/databases/VMdb/archives/Sunrise/Weld Controller/2000XWCProj/CyGOS/Arch/MCF/MCF5445x/MII.cpp_v   1.3   14 Apr 2014 07:34:28   rjamloki  $
//
...
//bool MII::GetLinkStatus()
//{
//	return false;//((ReadRegister(miiBMSR) & BMSR_LINK_STATUS) == BMSR_LINK_STATUS);
//}
//
//bool MII::ReadLinkStatus()
//{
//	return ((ReadRegister(miiBMSR) & BMSR_LINK_STATUS) == BMSR_LINK_STATUS);
//}

int MII::ReadSMIRegister(int reg)
{
#define MCF_FEC_MMFR_OP_SMI_READ   (0x00000000)
	int SMIFRAME16 = 0;
	int SMIFRAME32 = 0;
	// Clear the MII interrupt bit
	MCF_FEC_EIR(channel) = MCF_FEC_EIR_MII;
	// Write to the MII Management Frame Register to kick-off the MII read
	/*MCF_FEC_MMFR(channel) = MCF_FEC_MMFR_ST_01 | MCF_FEC_MMFR_OP_SMI_READ
			| MCF_FEC_MMFR_PA(address) | MCF_FEC_MMFR_RA(reg)
			| MCF_FEC_MMFR_TA_10;*/
	 SMIFRAME16 = (1 << 14) | (1 << 11) | (reg << 2) | (0x02) ;

	 SMIFRAME32 = (SMIFRAME16 << 16);
...
int MII::WriteSMIRegister(int reg, int value)
{
	int SMIFRAME16 = 0;
	int SMIFRAME32 = 0;
	// Clear the MII interrupt bit
