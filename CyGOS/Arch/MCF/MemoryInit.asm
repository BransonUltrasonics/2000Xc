| $Header:   D:/databases/VMdb/archives/Sunrise/Weld Controller/2000XWCProj/CyGOS/Arch/MCF/MemoryInit.asm_v   1.4   11 Dec 2014 08:31:08   AnPawar  $
|
...
MemoryInit:
				...
				move.l	#__CTOR_END,%D0
				sub.l	#__FLASH_RAM,%D0		| length in bytes
				lsr.l	#2,%D0					| now in longs
CodeCopyLoop:	move.l	(%A1)+,(%A0)+
				subq.l	#1,%D0
				bne.b	CodeCopyLoop

				| New code to put RAM in place
				move.l	#__DATA_START,%A0		| to
				move.l	#__TEXT_END,%A1			| from
				add.l	%D4,%A1					| in the flash
				move.l	#__DATA_END,%D0
				sub.l	#__DATA_START,%D0
				lsr.l	#2,%D0					| now in longs
DataCopyLoop:	move.l	(%A1)+,(%A0)+
				subq.l	#1,%D0
				bne.b	DataCopyLoop
