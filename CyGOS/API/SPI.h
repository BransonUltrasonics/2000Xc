// $Header:   D:/databases/VMdb/archives/Sunrise/Weld Controller/2000XWCProj/CyGOS/API/SPI.h_v   1.4   28 Nov 2014 08:11:32   rjamloki  $
//
...
public:
	...
	virtual void EnableSPI(bool phase, int bits, int clock, bool polarity =
			true, bool lsb = false, int ctar = 0) = 0;
	...
	virtual void SingleTransfer(int length, uint16 * data, int chipSelect,
			bool cont, int ctar = 0) = 0;
	virtual void LongWordTransfer(int length, uint8 * data, int maxLength,
			int chipSelect, int ctar = 0) = 0;
	virtual void SetDelayParams(int pcssck, int pasc, int pdt, int cssck,
			int asc, int dt, int ctar = 0)
	{
		pdt = 0;
		asc = 0;
		pcssck = cssck = 0;
