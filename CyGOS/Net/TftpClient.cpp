// $Header:   D:/databases/VMdb/archives/Sunrise/Weld Controller/2000XWCProj/CyGOS/Net/TftpClient.cpp_v   1.2   05 Mar 2014 13:58:38   rjamloki  $
//
...
int TftpClient::ReceiveFile(char * name, int * size, void * data, ip_addr server)
{
	TftpRequest request;
	request.name = name;
	request.data = (unsigned char *) data;
...
bool TftpClient::CheckOptions()
{
	for (int k = 0; k < NoOfOptions; k++)
	{
		...
		{
			int limit = (int) __USER_FLASH_SIZE;
			// transfer size greater than what we can accept
			if (atoi(values[k]) > limit)
			{
				char str[100];
				sprintf(str, "Transfer Size greater than %d bytes", limit);
