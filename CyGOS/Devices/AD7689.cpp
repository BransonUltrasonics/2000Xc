// $Header:   D:/databases/VMdb/archives/Sunrise/Weld Controller/2000XWCProj/CyGOS/Devices/AD7689.cpp_v   1.2   07 May 2014 10:48:56   rjamloki  $
//
...
AD7689::AD7689(SPI *spi, int cs, int ctar, int MaxChNumForSequencing)
{
   this->spi = spi;
   this->cs = 1 << cs;
   this->ctar = ctar;
   ...
   spi->EnableSPI(false, NUM_BITS_PERTRANSFER, SCK_FREQ, false, false, ctar);
   spi->SetDelayParams(0, 0, PDT, 0, 0, DT, ctar);
   EnableSequencing(MaxChNumForSequencing);
}
 ...
 * @return Voltage sensed on channel in raw
 */
int AD7689::ReadInput(int ch)
{
   int value = 0;
   pprintf("\n Not Implemented");
   ch = 0;
...
/**
 * Read specified channels in raw. Routine uses interrupt-driven SPI driver.
 ...
 * @param data Array of results in raw. All channel values are placed into
 * their indexed locations, if channel is skipped the index is skipped as
 * well.
 */
void AD7689::ReadInputs(int ch, int * mV)
{
   ch = 0;
   mV = 0;
   pprintf("\n Not Implemented");
...
void AD7689::EnableSequencing(int MaxChNum)
{
   ...
   SpiLock q(spi);
   q.Lock(-1);
   spi->SingleTransfer(NUM_TRANSFERS, data, cs, false, ctar);
   q.UnLock();

   MaxCh = MaxChNum + 1;
   printf("\n enable sequencing max max ch %d", MaxCh);
   if(ChVals == 0){
 ...
 */
void AD7689::ReadInputsThroughSequencing(int * RawVal)
{
   ...
   for(int i = 0; i < (MaxCh * NUM_TRANSFERS); i += NUM_TRANSFERS){
	  RawVal[i] = 0;
      spi->SingleTransfer(NUM_TRANSFERS, &ChVals[i], cs, false, ctar);
      RawVal[i] = ChVals[i];
   }
   q.UnLock();
}
