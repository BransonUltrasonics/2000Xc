// $Header:   D:/databases/VMdb/archives/Sunrise/Weld Controller/2000XWCProj/CyGOS/RTOS/RTOS.cpp_v   1.5   28 Nov 2014 08:10:14   rjamloki  $
//
...
static inline void ThroughputTest(int pin, bool value) {
	CPLD::SetDout(pin, value);
}
/**
...
void RTOS::Schedule()
{
	...
	{
		CriticalSection s;
		//TaskManager::UpdateTaskTime(taskCurrent->priority);
		taskHighest = 0;
		for (int i = 0; i < prioTotal; i++)
		{
			Task * task = tasks[i];
		...
		if ((intNesting == 0) && (taskHighest != taskCurrent)) {
			ASSERT(taskHighest->stackbegin[0] == (long)0xDEADBABE);
			//TaskManager::UpdateSystemTime(TaskManager::systimeSystem);
			// switch to new task
         CPU::ForceTaskSwitch();
		}
 ...
 */
void RTOS::TickHandler() {
	//TaskManager::UpdateTaskTime(taskHighest->priority);
	timestamp += Task::usecPerTick;
	if (tickHandler) {
		tickHandler();
	//	TaskManager::UpdateSystemTime(TaskManager::systimeHandler);
	}
	//TaskManager::UpdateSystemTime(TaskManager::systimeTimer);
	int prio;
	...
	{
		task->Tick();
		//TaskManager::UpdateTaskTime(task->priority);
		task = task->nextTick;
	}
	//TaskManager::UpdateSystemTime(TaskManager::systimeTimer);
	...
	for (prio = 0; prio < prioTotal; prio++)
	{
		...
		if (task)
		{
			...
			if ((uint32) task->stackbegin[0] != 0xDEADBABE)
...
void RTOS::PrintStatus(PrintStatusMode mode)
{
	...
	for (int prio = 0; prio < prioTotal; prio++)
	{
		...
		if (mode == PrintTasksWithStack)
			printf("Stack: %08X (%6d) ", (unsigned int)task->stack, (int)((task->stack
					- task->stackbegin) * 4));
		if (task->waitMutex)
		{
...
Task ** RTOS::Gettaskloadinformation()
{
	return tasks;
}

void RTOS::SetTickHandler(void(*handler)()) {
 tickHandler = handler;
}
