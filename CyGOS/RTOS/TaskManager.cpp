// $Header:   D:/databases/VMdb/archives/Sunrise/Weld Controller/2000XWCProj/CyGOS/RTOS/TaskManager.cpp_v   1.0   05 Mar 2014 15:53:18   rjamloki  $
//
...
TaskManager::Info TaskManager::systemData[TaskManager::systimeTotal];
...
int TaskManager::systemTime[TaskManager::systimeTotal];
...
int CyGOS::GetSystemTime(int time) {
	return TaskManager::GetSystemTime(time);
}
...
void TaskManager::Run() {
	...
	for (;;) {
		...
		for (int i = 0; i < systimeTotal; i++)
			systemTime[i] = RecalculateSysTime(i);
		ExitCritical(sr);
		taskManCallback();
	}
}
